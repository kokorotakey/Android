...
public class MainActivity extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		activity = this;
		etTermSearch = (EditText) findViewById(R.id.etTermSearch);
		btnSearch = (Button) findViewById(R.id.btSearch);
		Key = "COhvmmWGXAI5LjIlnKAcag0gF";
		Secret = "9T6rTC7brDZL9BpBYt27kBbDAXg1R51sSk7gC85W5mGRBxgfHk";
		 btnSearch.setOnClickListener(new OnClickListener(){

				@Override
				public void onClick(View arg0)
				{
				    SearchTerm=etTermSearch.getText().toString();
	...
	private class DownloadTwitterTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			// send the search rerult to the adapter for rendering
		...
		private String getSearchStream(String searchTerm) {
			...
			try {
				...
				String encodedUrl2 = URLEncoder.encode(" news source:twitterfeed", "UTF-8");
				results = getStream(TwitterSearchURL + encodedUrl+encodedUrl2);
			} catch (UnsupportedEncodingException ex) {
			} catch (IllegalStateException ex1) {
			}
			return results;
